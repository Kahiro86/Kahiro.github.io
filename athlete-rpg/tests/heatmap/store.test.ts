...
describe("applySessionToRollup — scoring", () => {
  ...
  it("a bodyweight exercise produces nonzero volume (regression: the pushup bug)", () => {
    const session: HeatmapSessionInput = {
      sets: [{ exerciseId: "pushup", reps: 15, bodyweightKg: BW, timestamp: MONDAY_NOON }],
    };
    applySessionToRollup(store, session);
    const row = store.get("chestSternal", weekStartFor(MONDAY_NOON));
    expect(row).toBeDefined();
    expect(row!.totalVolume).toBeGreaterThan(0);
  });

  it("warmup sets contribute volume just like any other set (no gating)", () => {
    const lightSet: LoggedSet = { exerciseId: "barbell-bench-press", weightKg: 20, reps: 10, bodyweightKg: BW, timestamp: MONDAY_NOON };
    applySessionToRollup(store, { sets: [lightSet] });
    const row = store.get("chestSternal", weekStartFor(MONDAY_NOON));
    expect(row!.totalVolume).toBeCloseTo(20 * 10 * 0.65 * 0.6); // chest share (.6) x sternal default split (.65)
  ...
  it("cardio-only week produces valid shares that sum to 100%", () => {
    applySessionToRollup(store, {
      sets: [],
      cardio: [{ modalityId: "run", durationMin: 30, intensityFactor: 1, timestampMs: MONDAY_NOON }],
    });
  ...
  it("cardio cap engages when a long session would otherwise dominate a muscle", () => {
    ...
    applySessionToRollup(store, {
      sets: [{ exerciseId: "leg-extension", weightKg: 20, reps: 5, bodyweightKg: BW, timestamp: MONDAY_NOON }],
    });
    const liftingRow = store.get("quads", weekStartFor(MONDAY_NOON))!;

    applySessionToRollup(store, {
      sets: [],
      cardio: [{ modalityId: "run", durationMin: 300, intensityFactor: 3, timestampMs: MONDAY_NOON + 1000 }],
    });
...
describe("rollup integrity", () => {
  ...
  it("session edit recomputes exactly one week, leaving other weeks untouched", () => {
    ...
    const otherBefore = store.get("chestSternal", otherWeekStart)!.totalVolume;
    ...
    const edited = store.get("chestSternal", weekStart)!;
    expect(edited.totalVolume).toBeCloseTo(60 * 10 * 0.6 * 0.65);
    expect(store.get("chestSternal", otherWeekStart)!.totalVolume).toBe(otherBefore);
  });
  ...
  it("lastTrainedAt tracks the most recent session, even if an earlier session is applied second (e.g. a backfilled log entry)", () => {
    ...
    expect(store.get("chestSternal", weekStart)!.lastTrainedAt).toBe(MONDAY_NOON + 2 * DAY);
  });
  ...
  it("a session spanning midnight lands entirely in one week bucket", () => {
    ...
    const session: HeatmapSessionInput = {
      sets: [
        { exerciseId: "barbell-bench-press", weightKg: 40, reps: 10, bodyweightKg: BW, timestamp: sundayNight },
        { exerciseId: "barbell-bench-press", weightKg: 40, reps: 10, bodyweightKg: BW, timestamp: mondayMorning },
      ],
    };
    applySessionToRollup(store, session);
    ...
    expect(store.get("chestSternal", sundayWeek)!.setCount).toBe(2);
    expect(store.get("chestSternal", mondayWeek)).toBeUndefined();
  });
});
