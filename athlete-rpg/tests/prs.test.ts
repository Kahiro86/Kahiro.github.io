...
function set(overrides: Partial<LoggedSet> = {}): LoggedSet {
  return { exerciseId: bench.id, timestamp: 0, ...overrides };
}

describe("detectPrs", () => {
  it("a brand-new exercise PRs on weight, reps, and volume simultaneously", () => {
    const prs = detectPrs(bench, set({ weightKg: 60, reps: 8 }), undefined, BW);
  ...
  it("does not PR when matching a previous best exactly", () => {
    ...
    history = recordSetIntoHistory(bench, set({ weightKg: 60, reps: 8 }), history, BW);

    const prs = detectPrs(bench, set({ weightKg: 60, reps: 8 }), history, BW);
    expect(prs).toEqual([]);
  });
  ...
  it("weight PR fires on a heavier load even at lower reps", () => {
    ...
    const prs = detectPrs(bench, set({ weightKg: 65, reps: 3 }), history, BW);
  ...
  it("rep PR requires more reps than ever achieved at that load or heavier", () => {
    let history = emptyExerciseHistory();
    history = recordSetIntoHistory(bench, set({ weightKg: 60, reps: 8 }), history, BW);
    history = recordSetIntoHistory(bench, set({ weightKg: 70, reps: 3 }), history, BW);
    ...
    const prs = detectPrs(bench, set({ weightKg: 65, reps: 6 }), history, BW);
    expect(prs.some((p) => p.type === "rep")).toBe(true);
  });

  it("volume PR fires whenever single-set volume exceeds the historical max", () => {
    let history = emptyExerciseHistory();
    history = recordSetIntoHistory(bench, set({ weightKg: 60, reps: 8 }), history, BW); // volume 480

    const prs = detectPrs(bench, set({ weightKg: 50, reps: 12 }), history, BW); // volume 600
  ...
  it("skips rep PRs for time-based exercises (no reps concept)", () => {
    ...
    const prs = detectPrs(plank, { exerciseId: plank.id, timestamp: 0, durationSec: 60 }, undefined, BW);
