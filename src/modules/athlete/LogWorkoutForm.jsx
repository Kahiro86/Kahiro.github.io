...
import { Trash2, X, Plus, CheckCircle } from "lucide-react";
import { B1, B2, BD, BD2, T1, T2, T3, GL, CY, PU, RE } from "../../shared/designTokens.js";
...
export function LogWorkoutForm({ onSave, onCancel }) {
  const [type, setType] = useState("strength");
  const [name, setName] = useState("");
  const [date, setDate] = useState(new Date().toISOString().split("T")[0]);
  const [exercises, setExercises] = useState([{ id: uidA(), name: "", sets: [{ reps: "", weight: "" }] }]);
  ...
  const setExName = (id, v) => setExercises((p) => p.map((e) => (e.id === id ? { ...e, name: v } : e)));
  const addSet = (id) => setExercises((p) => p.map((e) => (e.id === id ? { ...e, sets: [...e.sets, { reps: "", weight: "" }] } : e)));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "16px 22px", borderBottom: `1px solid ${BD}`, display: "flex", justifyContent: "space-between", alignItems: "center", background: B1 }}>
            ...
            {exercises.map((ex) => (
              <div key={ex.id} style={{ background: GL, border: `1px solid ${BD}`, borderRadius: 12, padding: "14px", marginBottom: 12 }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <select value={ex.name} onChange={(e) => setExName(ex.id, e.target.value)}
                ...
                {ex.name === "Custom" && (
                  <Inp value={ex.customName || ""} onChange={(v) => setExercises((p) => p.map((e) => (e.id === ex.id ? { ...e, customName: v, name: v || "Custom" } : e)))} placeholder="Exercise name" />
                )}
