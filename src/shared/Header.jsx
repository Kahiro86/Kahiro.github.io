...
import { Flame, Cpu, Menu, HelpCircle, Settings, Gem, Search } from "lucide-react";
...
export function Header({ module, aiOpen, onAIToggle, isMobile, onMenu, onNavigate, onOpenHelp, onOpenSettings, onOpenWhoIAm, onOpenSearch, streak = 0, xp = 0, level = 1, xpTitle = "", pctToNext = 0, toNext = 0, xpToday = 0, xpTodayByCat = {} }) {
  ...
  if (isMobile) {
    return (
      <div style={{ position: "relative", zIndex: 30, height: 56, background: "rgba(10,10,10,0.72)", backdropFilter: "blur(15px) saturate(125%)", WebkitBackdropFilter: "blur(15px) saturate(125%)", borderBottom: `1px solid ${BD}`, display: "flex", alignItems: "center", padding: "0 12px", gap: 10, flexShrink: 0 }}>
        ...
        {onOpenSearch && (
          <button onClick={onOpenSearch} aria-label="Search" title="Search (⌘K)" style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${BD}`, cursor: "pointer", background: GL, color: T2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <Search size={15} />
          </button>
        )}
        {onOpenWhoIAm && (
        ...
        {onOpenSettings && (
          <button onClick={onOpenSettings} aria-label="Settings" style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${BD}`, cursor: "pointer", background: GL, color: T2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <Settings size={15} />
          </button>
        )}
        <button onClick={onAIToggle} aria-label={`${appName} AI`} style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${aiOpen ? CY + "55" : BD}`, cursor: "pointer", background: aiOpen ? `linear-gradient(135deg,${CY}22,${PU}22)` : GL, color: aiOpen ? CY : T2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <Cpu size={15} />
        </button>
      </div>
    );
  ...
  return (
    <div style={{ position: "relative", zIndex: 30, height: 60, background: "rgba(10,10,10,0.72)", backdropFilter: "blur(15px) saturate(125%)", WebkitBackdropFilter: "blur(15px) saturate(125%)", borderBottom: `1px solid ${BD}`, display: "flex", alignItems: "center", padding: "0 22px", gap: 16, flexShrink: 0 }}>
      ...
      {onOpenSearch && (
        <button onClick={onOpenSearch} aria-label="Search" title="Search (⌘K)" style={{ width: 34, height: 34, borderRadius: 10, border: `1px solid ${BD}`, cursor: "pointer", background: GL, color: T2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <Search size={15} />
        </button>
      )}
      {onOpenWhoIAm && (
