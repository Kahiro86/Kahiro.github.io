...
//   · it's a marked rest or cheat day — always, in every mode. A rest day is a
//     planned day off, not a gap: it counts as done and never breaks the
//     streak or drags the rate.
//   · it's a protected (frozen) streak-insurance day.
//   · Hell mode is on and the day is genuinely full (isFullDay).
// Anything else returns undefined, so consistencyStats falls back to its normal
// "any activity that day" check.
import { isFullDay } from "./difficulty.js";
import { frozenSet } from "./streakInsurance.js";

export function consistencyOpts({ hell = false, habits = [], marks = {}, freezes = null } = {}) {
  const safe = new Set([...(marks?.rest || []), ...(marks?.cheat || [])]);
  const frozen = frozenSet(freezes);
  const base = hell ? (d) => isFullDay(habits, marks, d) : null;
  const isFull = (d) => {
    if (safe.has(d) || frozen.has(d)) return true;   // rest/cheat/protected → always a day done
    return base ? base(d) : undefined;               // undefined → engine falls back to anyXp
  };
  return { isFull };
}
