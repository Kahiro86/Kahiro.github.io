// ── FROZEN: the pre-revamp XP engine, kept only to compute a carry-forward ──
//
// This is the engine exactly as it stood before Gate 3, preserved so a user
// whose ledger has not been opened yet can have their historical total
// carried forward intact (criterion 21: never recompute history, never lose
// it either).
//
// Three things about this file:
//   · It is FROZEN. Nothing here is tuned, fixed or extended. It exists to
//     answer one question — "what had this person earned under the old
//     rules?" — and that answer must not drift.
//   · It is NOT an award path. It runs at most once per user, from the boot
//     migration, and never from a render. src/shared/xp/engine.js is the only
//     thing that prices an action (criterion 10).
//   · It is loaded lazily, so its value table never reaches the main bundle
//     for users who are already migrated.
//
// Recovered verbatim from commit fa8c295.
...
const RANKS = ["First Step", "Bronze", "Silver", "Gold", "Platinum", "Diamond", "Master", "Grandmaster", "Immortal"];
...
const JOURNEYS = BASE_JOURNEYS.map((j) => ({ ...j, tiers: extendTiers(j.tiers) }));
...
const ACHIEVEMENTS = JOURNEYS.flatMap((j) =>
...
/**
 * The historical total, under the rules that were live when it was earned.
 * The only export. Called once, from the boot migration, never from a render.
 */
export function legacyDerivedTotal(deps = {}) {
  try {
    const out = computeXpFrozen(deps);
    return Number.isFinite(out?.total) ? Math.max(0, Math.round(out.total)) : 0;
  } catch {
    return 0;   // a carry-forward that cannot be computed is 0, never a crash
  }
}
