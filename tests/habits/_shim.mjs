...
function makeMatchers(received, negate) {
  ...
  const m = {
    ...
    toMatch: (e) => ok(
      typeof received === "string" && (e instanceof RegExp ? e.test(received) : received.includes(e)),
      `expected ${fmt(received)} to match ${fmt(e)}`),
    toMatchObject: (e) => {
      const subset = (a, b) => {
        if (isAC(b)) return deepEqual(a, b);
        if (a && b && typeof a === "object" && typeof b === "object") {
          if (Array.isArray(b)) return Array.isArray(a) && a.length === b.length && b.every((x, i) => subset(a[i], x));
          return Object.keys(b).every((k) => subset(a[k], b[k]));
        }
        return deepEqual(a, b);
      };
      ok(subset(received, e), `expected ${fmt(received)} to match object ${fmt(e)}`);
    },
